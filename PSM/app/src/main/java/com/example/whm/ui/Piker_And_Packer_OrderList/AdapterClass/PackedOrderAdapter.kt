...
class PackedOrderAdapter(private val OderList: List<ModelClassOrderLIst>, var activity: Context?) :
    RecyclerView.Adapter<PackedOrderAdapter.ViewHolder>() {
    ...
    override fun getItemCount(): Int {
        if(OderList.size!=0){
            (activity as AppCompatActivity?)!!.supportActionBar!!.title = "Packed("+OderList.size+")"
        }else{
            (activity as AppCompatActivity?)!!.supportActionBar!!.title = "Packed("+OderList.size+")"
        }
