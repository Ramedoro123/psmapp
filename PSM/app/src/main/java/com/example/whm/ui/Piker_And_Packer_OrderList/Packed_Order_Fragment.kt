...
class Packed_Order_Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view=inflater.inflate(R.layout.fragment_packed__order_, container, false)
     ...
     var recyclerView: RecyclerView = view.findViewById(R.id.Packed_OrderListFragment_Packer)
        val sharedUnloadOrderPreferences =
            PreferenceManager.getDefaultSharedPreferences(this.context)
        var UnloadOrderCount = sharedUnloadOrderPreferences.getString("UnloadOrder", "[0]")
        setHasOptionsMenu(true)
        PackedlistAdapter = PackedOrderAdapter(orderList, this.context)
        val layoutManager = LinearLayoutManager(this.context)
        recyclerView.layoutManager = layoutManager
        recyclerView.itemAnimator = DefaultItemAnimator()
        recyclerView.adapter = PackedlistAdapter
        pack_Order();
        return view
    }

    private fun pack_Order() {
...
// Add the request to the RequestQueue.
    ...
    private fun PackedListData(
        ...
        orderList.add(order)
        PackedlistAdapter.notifyDataSetChanged()
    }
    }
